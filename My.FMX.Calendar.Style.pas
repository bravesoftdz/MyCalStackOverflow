...
{ TStyledCalendar }
  ...
  TMyStyledCalendar = class(TStyledPresentation)
  public const
...
procedure TMyStyledCalendar.ApplyStyle;
...
procedure TMyStyledCalendar.CalculateFirstDayOfWeek;
var
...
function TMyStyledCalendar.DefineItemIndexOfFirstDayInCurrentMonth: Integer;
var
  Interval: Integer;
  BeginDate: TDateTime;
begin
...
function TMyStyledCalendar.DefineModelClass: TDataModelClass;
begin
  Result := TCalendarModel;
end;

procedure TMyStyledCalendar.DoChange;
begin
  Model.DisableNotify;
  try
    Model.DateTime := Date;
...
procedure TMyStyledCalendar.DoDateSelected;
begin
  if Assigned(Model.OnDateSelected) then
    Model.OnDateSelected(PresentedControl);
end;

procedure TMyStyledCalendar.DoDayChange(Sender: TObject);
begin
...
procedure TMyStyledCalendar.DoDayClick(const Sender: TCustomListBox; const Item: TListBoxItem);
begin
  if Assigned(Model.OnDayClick) then
    Model.OnDayClick(PresentedControl);
end;

procedure TMyStyledCalendar.DoDaysMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Single);
begin
  if FDisableDayChange = 0 then
    Inc(FDisableDayChange);
end;

procedure TMyStyledCalendar.DoDaysMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Single);
begin
  if FDisableDayChange > 0 then
  begin
...
procedure TMyStyledCalendar.DoMonthChange(Sender: TObject);
var
...
procedure TMyStyledCalendar.DoNextClick(Sender: TObject);
begin
  Date := IncMonth(Date);
  DoChange;
end;

procedure TMyStyledCalendar.DoPrevClick(Sender: TObject);
begin
  Date := IncMonth(Date, -1);
  DoChange;
end;

procedure TMyStyledCalendar.DoRealign;
begin
  inherited;
  UpdateWeekWidth;
  UpdateWeeksDaysItemHeight;
...
procedure TMyStyledCalendar.DoTodayClick(Sender: TObject);
begin
  Date := Now;
  DoChange;
  if FDisableDayChange = 0 then
...
procedure TMyStyledCalendar.DoYearChange(Sender: TObject);
var
  AYear, AMonth, ADay: Word;
begin
  DecodeDate(FDateTime, AYear, AMonth, ADay);
...
procedure TMyStyledCalendar.FillDays;
const
...
procedure TMyStyledCalendar.FillCalendar;
begin
  if IsUpdating then
    Exit;
...
procedure TMyStyledCalendar.UpdateWeeksDaysItemHeight;
const
  DaysLines = 6;
begin
  if FDays <> nil then
...
procedure TMyStyledCalendar.FillWeekDays;
var
  I: Integer;
  Day: Integer;
...
procedure TMyStyledCalendar.FillWeeksNumbers;
var
  I: Integer;
  CaptionControl: ICaption;
...
procedure TMyStyledCalendar.FillYears;
const
...
procedure TMyStyledCalendar.FreeStyle;
begin
  inherited;
  if FPrev <> nil then
    FPrev.OnClick := nil;
...
function TMyStyledCalendar.GetDate: TDate;
begin
  Result := Trunc(FDateTime);
end;

function TMyStyledCalendar.GetModel: TCalendarModel;
begin
  Result := inherited GetModel<TCalendarModel>;
end;

procedure TMyStyledCalendar.KeyDown(var Key: Word; var KeyChar: WideChar; Shift: TShiftState);
begin
  inherited;
  case Key of
    vkLeft:
...
procedure TMyStyledCalendar.MouseWheel(Shift: TShiftState; WheelDelta: Integer; var Handled: Boolean);
begin
  inherited;
  if not Handled then
  begin
...
procedure TMyStyledCalendar.PMInit(var AMessage: TDispatchMessage);
begin
  BeginUpdate;
  try
    Date := Model.DateTime;
...
procedure TMyStyledCalendar.SetDate(Value: TDate);
begin
  FDisableDayChange := FDisableDayChange + 1;
  try
...
procedure TMyStyledCalendar.SetDateTime(Value: TDateTime);
begin
...
function TMyStyledCalendar.TryFindDayItem(const ADateTime: TDateTime): TListBoxItem;
var
...
procedure TMyStyledCalendar.UpdateWeekWidth;
begin
  if FWeek <> nil then
    FWeek.ItemWidth := Floor(FWeek.Width / DaysPerWeek);
end;

procedure TMyStyledCalendar.MMDateChanged(var AMessage: TDispatchMessageWithValue<TDateTime>);
begin
  SetDateTime(AMessage.Value);
end;

procedure TMyStyledCalendar.MMFirstDayOfWeekChanged(var AMessage: TDispatchMessageWithValue<TCalDayOfWeek>);
begin
  CalculateFirstDayOfWeek;
  FillCalendar;
end;

procedure TMyStyledCalendar.MMWeekNumbersChanged(var AMessage: TDispatchMessageWithValue<Boolean>);
begin
  FillCalendar;
  Realign;
end;
